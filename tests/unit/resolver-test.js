...
module('Unit | Resolver | default', (hooks) => {
  ...
  test('it reopens module with properties when resolved', function (assert) {
    ...
    assert.strictEqual(
      resolved.PrototypeMixin.mixins.length,
      2,
      'reopen is included'
    );
    assert.strictEqual(
      resolved.PrototypeMixin.mixins[0].properties,
      reopen.reopen[0],
      'reopen matches with first defined'
    );
    assert.strictEqual(
      resolved.PrototypeMixin.mixins[1].properties,
      reopen.reopen[1],
      'reopen matches with second defined'
    );
  });
  ...
  test('it reopens module with mixin when resolved', function (assert) {
    ...
    assert.strictEqual(
      resolved.PrototypeMixin.mixins[0],
      reopen.reopen[0],
      'reopen matches with defined'
    );
  });
  ...
  test('it reopens class with mixin when resolved', function (assert) {
    const moduleName = 'dummy/object';
    const reopenModule = 'dummy/reopens/object';
    const object = EmberObject.extend();
    const reopen = {
    ...
    assert.strictEqual(resolved.foo, 'bar', 'reopen matches with defined');
    assert.strictEqual(resolved.bar, 'foo', 'reopen matches with defined');
  });
});
