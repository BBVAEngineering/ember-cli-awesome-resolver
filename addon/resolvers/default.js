...
function parseName(fullName) {
  // istanbul ignore if: fix for ember cli resolver integration (?)
 ...
 * @public
 */
export default class AwesomeResolver extends Resolver {
  /**
   ...
   * @type Array
   */
  namespaces = this.namespaces || null;
   ...
   * @type Boolean
   */
  moduleBasedResolver = this.moduleBasedResolver || true;
  ...
  init() {
    ...
    if (!this.namespaces) {
      this.namespaces = A([this.namespace.modulePrefix]);
    }
  ...
  lookupDescription(fullName) {
    ...
    return (
      ...
      makeToString(parsedName.root, parsedName.type, parsedName.name)
    );
  }
  ...
  knownForType(type) {
    ...
    Object.assign(items, super.knownForType(type));
  ...
  makeToString(factory, fullName) {
    ...
    let stringName = super.makeToString(factory, fullName);
    ...
    if (factory._moduleName) {
      ...
      stringName = makeToString(
        formatedModule.namespace,
        formatedModule.type,
        formatedModule.name
      );
    }
  ...
  findModule(namespace, type, name) {
    const resolvers = A([
      this.findModuleByMain,
      this.findModuleByType,
      this.findModuleByPod,
    ]);
